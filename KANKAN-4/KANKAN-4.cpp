...
void AreasOfTriangles() {
	...
	auto models3 = std::make_unique<double[]>(nU3);

	auto deltas3 = std::make_unique<double[]>(nU3);
	...
	auto derivatives3 = std::make_unique<std::unique_ptr<double[]>[]>(nU3);
	for (int i = 0; i < nU3; ++i) {
		derivatives3[i] = std::make_unique<double[]>(nU2);
	}
	...
	for (int epoch = 0; epoch < 128; ++epoch) {
		for (int i = 0; i < nTrainingRecords; ++i) {
			...
			layer3->ComputeDeltas(derivatives3, deltas3, deltas2, nU2, nU3);
